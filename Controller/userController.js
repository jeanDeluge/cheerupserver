...
module.exports = {
  join: async (request, response) => {
    ...
    try {
      ...
      let messageWithToken = {
        ...
        html:
          "" +
          `<div><a href ="${host}+"/confirmEmail/"+${tokenForSignUp}" ></a> <div>`,
      };
      //
      ...
      if (!create) {
        ...
        response
          .status(200)
          .json({ message: "mail send  mail 인증부탁드립니다." });
      } else {
  ...
  confirmMail: (request, response) => {
    const token =
      request.headers["x-access-join-token"] || request.headers.token;
  ...
  login: async (request, response) => {
    ...
    try {
      ...
      console.log(user, "유저 생일");
      response.status(200).json("로그인됨");
      /*일
            }).then(user=>{
                if(!user){
                    response.status(403).json("user does not exist")
